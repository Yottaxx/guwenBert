...
from gwMultiChoices import gwBert
...
tokenizer = AutoTokenizer.from_pretrained(CFG['model'])  # 加载bert的分词器
...
def collate_fn(data):  # b * numC  *dim
...
for fold in range(int(CFG['fold_num'])):
    ...
    model = gwBert(CFG['model']).to(device)  # 模型
...
test_loader = DataLoader(test_set, batch_size=CFG['valid_bs'], collate_fn=collate_fn, shuffle=False,
                         num_workers=CFG['num_workers'])
model = gwBert(CFG['model']).to(device)
model = nn.DataParallel(model)
